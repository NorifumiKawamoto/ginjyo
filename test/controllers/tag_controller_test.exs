defmodule Ginjyo.TagControllerTest do
  use Ginjyo.ConnCase

  alias Ginjyo.Factory
  alias Ginjyo.Tag
  @valid_attrs %{name: "some content", slug: "somecontent"}
  @invalid_attrs %{}

  setup do
    conn = conn()
    role = Factory.create(:role, rank: 10)
  ...
  test "renders form for editing chosen resource", %{conn: conn, tag: tag} do
    conn = get conn, tag_path(conn, :edit, tag)
    assert html_response(conn, 200) =~ "Edit tag"
  end

  test "updates chosen resource and redirects when data is valid", %{conn: conn, tag: tag} do
  ...
  test "deletes chosen resource", %{conn: conn, tag: tag} do
